...
describe('Signup page', function() {
    var httpBackendMock = function() {
        angular.module('httpBackendMock', ['yunakQuiz', 'ngMockE2E']).run(function($httpBackend) {
          var user = {username: 'Inokentiy', password: '12345678', password_confirmation: '12345678', email: 'ewrrg@fgfgf.com', birthday: '2014-10-08'};
          var userok = {username: 'Igoryan', password: '12345678', password_confirmation: '12345678', email: 'ewrrg@fgfgf.com', birthday: '2014-10-08'};

          $httpBackend.whenPOST('http://localhost:9292/user', user).respond(function(url, data, headers) {
            console.log('Received these data:', url, data, headers);
            return [400, {"username":"has already been taken"}, {}];
          });

          $httpBackend.whenPUT('http://localhost:9292/user', userok).respond(function(method, url, data, headers) {
  ...
  beforeEach(function() {
      browser.driver.get('http://localhost:8000/#/auth/signup');
  ...
  describe('Incorrect registration', function() {
    ...
    it('should display error message when we submit empty fields', function() {
        ...
        expect(element(by.css('[name=username]+div')).getText()).toBeTruthy();
        expect(element(by.css('[name=password]+div')).getText()).toBeTruthy();
        expect(element(by.css('[name=email]+div')).getText()).toBeTruthy();
        expect(element(by.css('[name=birthday]+div')).getText()).toBeTruthy();
    });
    ...
    it('should display error message when password confirmation is fail', function() {
        ...
        expect(element(by.css('[name=password_confirmation]+div')).getText()).toBeTruthy();
    });
    ...
    it('should display error message when Username has already been taken', function() {
        ...
        xit('should display error message when Username has already been taken', function() {
        var captcha = element(by.binding('reg.captcha')).getText()
        element(by.css('[name=email]')).sendKeys("ewrrg@fgfgf.com");
